        ...
        Try
            ...
            Using process As Process = Process.Start(processInfo)
                ...
                If process.ExitCode = 0 Then
                    Dim finalFile As String = backupFile
                    ...
                    ' Hapus backup lama jika lebih dari 5 file di folder hari ini
                    Dim ext As String = If(typedata.ToUpper() = "SQL", "*.sql", "*.zip")
                    Dim files As FileInfo() = New DirectoryInfo(backupFolder).GetFiles(ext) _
                        .OrderBy(Function(f) f.LastWriteTime) _
                        .ToArray()

                    If files.Length > 5 Then
                        For i As Integer = 0 To files.Length - 6
                            files(i).Delete()
                        Next
                    End If
