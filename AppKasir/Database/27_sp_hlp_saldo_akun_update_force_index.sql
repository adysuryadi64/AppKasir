-- =============================================================================
-- 27: Patch sp_hlp_saldo_akun_update — FORCE INDEX untuk covering index
-- =============================================================================
-- Masalah: MySQL optimizer memilih idx_nomor_akun_d_jurnal (scan ratusan ribu baris)
--          padahal covering index idx_covering_akun_d lebih cepat 5x karena
--          NOMINAL sudah ada di index — tidak perlu baca data row.
--
-- Solusi: Tambah FORCE INDEX(idx_covering_akun_d/k) di kedua subquery SUM.
--
-- Dampak: Semua pemanggilan UpdateSaldoAkun() di VB.NET, SP lain, dan PHP
--         otomatis lebih cepat — logika tidak berubah sama sekali.
--
-- Benchmark (db_moroseneng, 627k baris JurnalUmum):
--   Sebelum: ~7.6 detik untuk 4 akun
--   Sesudah: ~3 detik untuk 4 akun (sebelum sp_hlp_saldo_akun_delta tersedia)
--
-- Catatan: sp_hlp_saldo_akun_update masih dipakai sebagai fallback dan untuk
--          rekonsiliasi. Untuk transaksi realtime, gunakan sp_hlp_saldo_akun_delta.
-- =============================================================================
...
CREATE PROCEDURE sp_hlp_saldo_akun_update(IN p_kode_akun VARCHAR(20))
BEGIN
    ...
    IF p_kode_akun IS NOT NULL AND p_kode_akun <> '' THEN
        ...
        LEFT JOIN (
            ...
            FROM JurnalUmum FORCE INDEX(idx_covering_akun_d)
        ...
        LEFT JOIN (
            ...
            FROM JurnalUmum FORCE INDEX(idx_covering_akun_k)
        ...
        SET SALDO_SEBELUMNYA = v_laba_rugi,
