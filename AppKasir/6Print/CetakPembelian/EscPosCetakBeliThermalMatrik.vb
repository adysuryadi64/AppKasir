' ================================================================
' EscPosCetakBeliThermalMatrik
' Cetak nota pembelian via ESC/POS (Thermal & Dot Matrix).
' ================================================================
Public Class EscPosCetakBeliThermalMatrik
...
#Region "Cetak Thermal"
    Public Sub CetakThermal()
        ...
        If _cfg.ModeCetak = "GDI+ (Windows Print)" Then
            Dim c As New GdiCetakBeliThermalMatrik() : c.Cetak() : Exit Sub
        End If
        For i As Integer = 1 To _cfg.JumlahCetak
            ...
            Select Case _cfg.ModelStruk
                ...
                Case "Model 3 Dengan Total Hutang" : CetakModel4(esc)
    ...
    Private Sub CetakModel4(esc As PrinterEscPos)
        ' Model 3 Dengan Total Hutang — ringkasan hutang supplier
        CetakHeader(esc) : CetakInfo(esc) : CetakItem(esc)
        CetakTotalDenganHutangSupplier(esc) : CetakFooter(esc)
    End Sub
    ...
    Private Sub CetakInfo(esc As PrinterEscPos)
        ...
        esc.CetakGaris()
        esc.CetakBaris(FmtLabel("No Beli  ", ": " & Beli_IdPembelian, n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Nota     ", ": " & Beli_NotaPembelian, n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Tanggal  ", ": " & Beli_Tanggal.ToString("yyyy-MM-dd HH:mm:ss"), n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Kasir    ", ": " & Beli_IdUser & " - " & Beli_IdKomputer, n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Supplier ", ": " & Beli_NamaSupplier, n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Lokasi   ", ": " & Beli_Lokasi, n), _cfg.EscUkuranKeterangan)
        esc.CetakGaris()
        esc.CetakTengah("NOTA PEMBELIAN", _cfg.EscUkuranKeterangan)
        esc.CetakGaris()
    End Sub
    ...
    Private Sub CetakInfoSingkat(esc As PrinterEscPos)
        ...
        esc.CetakGaris()
        esc.CetakBaris(FmtLabel("No Beli  ", ": " & Beli_IdPembelian, n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Tgl      ", ": " & Beli_Tanggal.ToString("yyyy-MM-dd HH:mm:ss"), n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Supplier ", ": " & Beli_NamaSupplier, n), _cfg.EscUkuranKeterangan)
        esc.CetakGaris()
        esc.CetakTengah("NOTA PEMBELIAN", _cfg.EscUkuranKeterangan)
        esc.CetakGaris()
    ...
    Private Sub CetakItem(esc As PrinterEscPos)
        ...
        esc.CetakBaris(RataKiri("Nama Barang", prefixHeader) &
                       RataKanan("Harga", lebarHarga) & RataKanan("Jml", lebarJml), _cfg.EscUkuranIsi)
        esc.CetakGaris()
        For Each item As ItemNotaBeli In Beli_DaftarItem
            ...
            esc.CetakBaris(
                ...
                RataKanan(Rp(item.HargaBeli), lebarHarga) &
    ...
    Private Sub CetakTotal(esc As PrinterEscPos)
        ...
        esc.CetakBaris((Beli_DaftarItem.Count & " item").PadRight(posLabel) &
            RataKanan("Total :", lebarLbl) & RataKanan(Rp(Beli_Tagihan), lebarNilai))
        esc.CetakBaris("".PadRight(posLabel) &
            RataKanan("Tunai :", lebarLbl) & RataKanan(Rp(Beli_Pembayaran), lebarNilai))
        If Beli_NominalTransfer > 0 Then
            Dim lblTf As String = If(String.IsNullOrEmpty(Beli_NamaAkunTf), "Transfer", Beli_NamaAkunTf)
            esc.CetakBaris("".PadRight(posLabel) &
                RataKanan(lblTf & " :", lebarLbl) & RataKanan(Rp(Beli_NominalTransfer), lebarNilai))
        End If
        esc.CetakBaris("".PadRight(posLabel) & New String("="c, lebarKanan))
        If Beli_NominalBayar > 0 Then
            esc.CetakBaris("".PadRight(posLabel) &
                RataKanan("Hutang :", lebarLbl) & RataKanan(Rp(Beli_NominalBayar), lebarNilai))
            If Beli_JatuhTempo > Date.MinValue Then
                esc.CetakBaris("".PadRight(posLabel) &
                    RataKanan("Jatuh Tempo :", lebarLbl) & RataKanan(Beli_JatuhTempo.ToString("dd-MM-yyyy"), lebarNilai))
            End If
        End If
        esc.CetakGaris()
        esc.CetakBaris(FmtLabel("Status   ", ": " & Beli_StatusTransaksi, n))
        esc.CetakGaris()
    End Sub

    Private Sub CetakTotalDenganHutangSupplier(esc As PrinterEscPos)
        Dim n As Integer = esc.JumlahKarakterPerBaris
        Dim posLabel As Integer = CInt(n * 0.65)
        Dim lebarKanan As Integer = n - posLabel
        Dim lebarNilai As Integer = CInt(lebarKanan * 0.55)
        ...
        esc.CetakGaris()
        esc.CetakBaris(KiriKanan("Sisa Hutang Supplier", Rp(Beli_HutangAkhir), n))
        esc.CetakGaris()
    End Sub
...
#Region "Cetak Dot Matrix"
    ...
    Private Function BangunEscP() As Byte()
        ...
        baris.Add(pI & garis)
        baris.Add(pI & FmtLabel("No Beli  ", ": " & Beli_IdPembelian, nI))
        baris.Add(pI & FmtLabel("Nota     ", ": " & Beli_NotaPembelian, nI))
        baris.Add(pI & FmtLabel("Tanggal  ", ": " & Beli_Tanggal.ToString("yyyy-MM-dd HH:mm:ss"), nI))
        baris.Add(pI & FmtLabel("Kasir    ", ": " & Beli_IdUser & " - " & Beli_IdKomputer, nI))
        baris.Add(pI & FmtLabel("Supplier ", ": " & Beli_NamaSupplier, nI))
        baris.Add(pI & FmtLabel("Lokasi   ", ": " & Beli_Lokasi, nI))
        baris.Add(pI & garis)
        baris.Add(pK & "NOTA PEMBELIAN".PadLeft((nK + "NOTA PEMBELIAN".Length) \ 2))
        baris.Add(pI & garis)
        ...
        For Each item As ItemNotaBeli In Beli_DaftarItem
            ...
            baris.Add(pI & RataKiri(no.ToString(), lebarNo) & RataKiri(nama, lebarNama) &
                      ...
                      RataKanan(BeliRp(item.HargaBeli), lebarHarga) &
                      RataKanan(BeliRp(item.Total), lebarJml))
            no += 1
        Next
        baris.Add(pI & garis)
        Dim lebarJmlTotal As Integer = CInt(nI * 0.22)
        Dim lebarLblTotal As Integer = 7
        baris.Add(pI & (Beli_DaftarItem.Count & " item").PadRight(nI - lebarLblTotal - lebarJmlTotal) &
                  RataKanan("Total :", lebarLblTotal) & RataKanan(BeliRp(Beli_Tagihan), lebarJmlTotal))
        baris.Add(pI & "".PadRight(nI - lebarLblTotal - lebarJmlTotal) &
                  RataKanan("Bayar :", lebarLblTotal) & RataKanan(BeliRp(Beli_Pembayaran), lebarJmlTotal))
        baris.Add(pI & "".PadRight(nI - lebarLblTotal - lebarJmlTotal) & New String("="c, lebarLblTotal + lebarJmlTotal))
        If Beli_Tagihan > Beli_Pembayaran Then
            baris.Add(pI & "".PadRight(nI - lebarLblTotal - lebarJmlTotal) &
                      RataKanan("Hutang:", lebarLblTotal) & RataKanan(BeliRp(Beli_Tagihan - Beli_Pembayaran), lebarJmlTotal))
        End If
        baris.Add(pI & garis)
        baris.Add(pI & FmtLabel("Status   ", ": " & Beli_StatusTransaksi, nI))
        baris.Add(pI & garis)
