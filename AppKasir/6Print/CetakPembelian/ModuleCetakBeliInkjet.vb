...
' ================================================================
' ModuleCetakBeliInkjet — Cetak nota pembelian ke Inkjet/Laser
' ================================================================
Module ModuleCetakBeliInkjet
    ...
    Public Sub CetakNota()
        _cfg = New KonfigurasiInkjet("Beli")
    ...
    Private Sub Pd_PrintPage(sender As Object, e As PrintPageEventArgs)
        ...
        _xNo = b.Left
        _xNama = b.Left + CInt(w * _cfg.PctKolomNo / 100.0)
        _xQty = b.Left + CInt(w * (_cfg.PctKolomNo + (100 - _cfg.PctKolomNo - _cfg.PctKolomQty - _cfg.PctKolomHarga * 2)) / 100.0)
        _xHarga = b.Left + CInt(w * (_cfg.PctKolomNo + (100 - _cfg.PctKolomNo - _cfg.PctKolomQty - _cfg.PctKolomHarga * 2) + _cfg.PctKolomQty) / 100.0)
        _xJml = b.Right
        _xKanan = b.Right
        ...
        g.DrawString("NOTA PEMBELIAN", New Font(_cfg.FontJudul, _cfg.UkuranJudul + 1, FontStyle.Bold), Brushes.Black, tengah, y, fmtTengah)
        ...
        g.DrawString("No. Beli", fIsi, Brushes.Black, xKiri, y)
        g.DrawString(": " & Beli_IdPembelian, fBold, Brushes.Black, xVal1, y)
        g.DrawString("Kasir", fIsi, Brushes.Black, xKanan2, y)
        g.DrawString(": " & Beli_IdUser & " / " & Beli_IdKomputer, fIsi, Brushes.Black, xVal2, y) : y += lh
        g.DrawString("Tanggal", fIsi, Brushes.Black, xKiri, y)
        g.DrawString(": " & Beli_Tanggal.ToString("dd-MM-yyyy HH:mm:ss"), fIsi, Brushes.Black, xVal1, y)
        g.DrawString("Lokasi", fIsi, Brushes.Black, xKanan2, y)
        g.DrawString(": " & Beli_Lokasi, fIsi, Brushes.Black, xVal2, y) : y += lh
        g.DrawString("Supplier", fIsi, Brushes.Black, xKiri, y)
        g.DrawString(": " & Beli_NamaSupplier, fIsi, Brushes.Black, xVal1, y) : y += lh
        If Not String.IsNullOrEmpty(Beli_AlamatSupplier) Then
            g.DrawString("Alamat", fIsi, Brushes.Black, xKiri, y)
            g.DrawString(": " & Beli_AlamatSupplier, fIsi, Brushes.Black, xVal1, y) : y += lh
        End If
        g.DrawString("Nota", fIsi, Brushes.Black, xKiri, y)
        g.DrawString(": " & Beli_NotaPembelian, fIsi, Brushes.Black, xVal1, y) : y += lh + 4
        ...
        g.DrawString("Harga Beli", fBold, Brushes.Black, _xHarga, y, fmtKanan)
        ...
        For Each item As ItemNotaBeli In Beli_DaftarItem
            ...
            g.DrawString(item.HargaBeli.ToString("N0", cultureIndonesia), fIsi, Brushes.Black, _xHarga, y, fmtKanan)
        ...
        Dim TulisLbl = Sub(lbl As String, fnt As Font, yy As Integer)
                       ...
                       End Sub
        g.DrawString(Beli_DaftarItem.Count & " item", fKecil, Brushes.Gray, b.Left, y)
        TulisLbl("Total :", fIsi, y)
        g.DrawString(Beli_Tagihan.ToString("N0", cultureIndonesia), fIsi, Brushes.Black, _xJml, y, fmtKanan) : y += lh
        TulisLbl("Tunai :", fIsi, y)
        g.DrawString(Beli_Pembayaran.ToString("N0", cultureIndonesia), fIsi, Brushes.Black, _xJml, y, fmtKanan) : y += lh
        If Beli_NominalTransfer > 0 Then
            Dim lblTf As String = If(String.IsNullOrEmpty(Beli_NamaAkunTf), "Transfer", Beli_NamaAkunTf)
            TulisLbl(lblTf & " :", fIsi, y)
            g.DrawString(Beli_NominalTransfer.ToString("N0", cultureIndonesia), fIsi, Brushes.Black, _xJml, y, fmtKanan) : y += lh
        End If
        If Beli_NominalBayar > 0 Then
            g.DrawLine(New Pen(Color.Black, 2), xLbl, y, _xJml, y) : y += 4
            TulisLbl("Hutang :", fBold, y)
            g.DrawString(Beli_NominalBayar.ToString("N0", cultureIndonesia), fBold, Brushes.Black, _xJml, y, fmtKanan) : y += lh
            If Beli_JatuhTempo > Date.MinValue Then
                g.DrawString("Jatuh Tempo : " & Beli_JatuhTempo.ToString("dd-MM-yyyy"), fKecil, Brushes.Gray, b.Left, y) : y += lh
            End If
        Else
            g.DrawLine(New Pen(Color.Black, 2), xLbl, y, _xJml, y) : y += 4
        End If
        g.DrawString("Status : " & Beli_StatusTransaksi, fBold, Brushes.Black, b.Left, y) : y += lh + 4
        g.DrawLine(Pens.LightGray, b.Left, y, _xKanan, y) : y += 4
        g.DrawString("Terbilang: " & Terbilang(Beli_Tagihan) & " Rupiah",
                     New Font(_cfg.FontIsi, Math.Max(6, _cfg.UkuranIsi - 1), FontStyle.Italic), Brushes.Gray, b.Left, y) : y += lh + 8

        ' Model 2 — sisa hutang supplier dari tbl_supliyer
        If _cfg.ModelNota = "Model 2 Dengan Total Hutang" Then
            g.DrawLine(New Pen(Color.Black, 1), b.Left, y, _xKanan, y) : y += 4
            Dim xLblH As Integer = b.Left + CInt(b.Width * 0.55)
            g.DrawString("Sisa Hutang Supplier :", fBold, Brushes.Black,
                         New RectangleF(b.Left, y, xLblH - b.Left, fBold.GetHeight(g) + 2), fmtLbl)
            g.DrawString(Beli_HutangAkhir.ToString("N0", cultureIndonesia), fBold, Brushes.Black, _xJml, y, fmtKanan) : y += lh + 4
            g.DrawLine(Pens.LightGray, b.Left, y, _xKanan, y) : y += 4
        End If
        ...
        If _cfg.TampilTandaTangan Then
            ...
            g.DrawString("( " & Beli_IdUser & " )", fKecil, Brushes.Black, xT2, y, fmtTengah) : y += lh + 8
        End If
