...
Public Class GdiCetakBayarHutang
    ...
    Public Sub New()
        _cfg = New KonfigurasiThermal("BayarHutang")
    End Sub
    ...
    Public Sub Cetak()
        ...
        Next
        If _cfg.KodeLaciKasir <> "(Tidak Ada)" Then BukaLaciKasir("BayarHutang")
    End Sub
    ...
    Private Sub HitungPanjang()
        _panjangKertas = BH_DaftarDetail.Count * 25 + 350
    End Sub
    ...
    Private Sub CetakHalaman(e As PrintPageEventArgs)
        ...
        g.DrawString("No Bayar", FKet, Brushes.Black, BK, y) : g.DrawString(": " & BH_NoBayar, FKet, Brushes.Black, pv, y) : y += 10 + Jarak
        g.DrawString("Tanggal", FKet, Brushes.Black, BK, y) : g.DrawString(": " & BH_Tanggal.ToString("yyyy-MM-dd HH:mm:ss"), FKet, Brushes.Black, pv, y) : y += 10 + Jarak
        g.DrawString("Kasir", FKet, Brushes.Black, BK, y) : g.DrawString(": " & BH_IdUser & " - " & BH_IdKomputer, FKet, Brushes.Black, pv, y) : y += 10 + Jarak
        g.DrawString("Supplier", FKet, Brushes.Black, BK, y) : g.DrawString(": " & BH_NamaSupplier, FKet, Brushes.Black, pv, y) : y += 14 + Jarak
        g.DrawString(Garis, FGaris, Brushes.Black, BK, y) : y += 10 + Jarak
        g.DrawString("BUKTI BAYAR HUTANG", New Font(_cfg.FontKeterangan, _cfg.UkuranKeterangan + 2), Brushes.Black, Tengah, y, fmtTengah) : y += 14 + Jarak
        ...
        g.DrawString("No Faktur Beli", FIsi, Brushes.Black, BK, y)
        ...
        For Each item As ItemBayarHutang In BH_DaftarDetail
            y += 10 + Jarak
            g.DrawString(item.IdBeli, FIsi, Brushes.Black, BK, y)
            ...
            y += 10 + Jarak
            g.DrawString("  Tgl: " & item.TanggalBeli.ToString("dd-MM-yyyy") & "  Status: " & item.Status,
        ...
        ' Total
        g.DrawString("Total Hutang :", FIsi, Brushes.Black, m3, y, fmtKanan)
        g.DrawString(Rp(BH_TotalHutang), FIsi, Brushes.Black, m5, y, fmtKanan) : y += 10 + Jarak
        g.DrawString("Dibayar      :", FIsi, Brushes.Black, m3, y, fmtKanan)
        g.DrawString(Rp(BH_NominalBayar), FIsi, Brushes.Black, m5, y, fmtKanan) : y += 10 + Jarak
        g.DrawString(BuatGarisGanda(HitungLebarGaris(_cfg.LebarKertas)), FGaris, Brushes.Black, m3, y) : y += 10 + Jarak
        g.DrawString("Sisa Hutang  :", New Font(_cfg.FontIsi, _cfg.UkuranIsi, FontStyle.Bold), Brushes.Black, m3, y, fmtKanan)
        g.DrawString(Rp(BH_SisaHutang), New Font(_cfg.FontIsi, _cfg.UkuranIsi, FontStyle.Bold), Brushes.Black, m5, y, fmtKanan) : y += 10 + Jarak
        g.DrawString(Garis, FGaris, Brushes.Black, BK, y) : y += 10 + Jarak
