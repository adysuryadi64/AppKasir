' ================================================================
' EscPosCetakBayarHutang — ESC/POS thermal & dot matrix bukti bayar hutang
' ================================================================
Public Class EscPosCetakBayarHutang
...
#Region "Cetak Thermal"
    Public Sub CetakThermal()
        ...
        If _cfg.ModeCetak = "GDI+ (Windows Print)" Then
            Dim c As New GdiCetakBayarHutang() : c.Cetak() : Exit Sub
        End If
    ...
    Private Sub CetakModel1(esc As PrinterEscPos)
        ...
        esc.CetakGaris()
        esc.CetakBaris(FmtLabel("No Bayar ", ": " & BH_NoBayar, n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Tanggal  ", ": " & BH_Tanggal.ToString("yyyy-MM-dd HH:mm:ss"), n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Kasir    ", ": " & BH_IdUser & " - " & BH_IdKomputer, n), _cfg.EscUkuranKeterangan)
        esc.CetakBaris(FmtLabel("Supplier ", ": " & BH_NamaSupplier, n), _cfg.EscUkuranKeterangan)
        esc.CetakGaris()
        esc.CetakTengah("BUKTI BAYAR HUTANG", _cfg.EscUkuranKeterangan)
        esc.CetakGaris()
        ...
        esc.CetakBaris(RataKiri("No Faktur Beli", lebarHarga) & RataKanan("Bayar", lebarBayar), _cfg.EscUkuranIsi)
        esc.CetakGaris()
        For Each item As ItemBayarHutang In BH_DaftarDetail
            esc.CetakBaris(RataKiri(item.IdBeli, lebarHarga) & RataKanan(Rp(item.Pembayaran), lebarBayar), _cfg.EscUkuranIsi)
            esc.CetakBaris("  " & item.TanggalBeli.ToString("dd-MM-yyyy") & "  Sisa: " & Rp(item.Sisa), _cfg.EscUkuranIsi)
        Next
        esc.CetakGaris()
        ...
        esc.CetakBaris("".PadRight(posLabel) & RataKanan("Total Hutang :", lebarLbl) & RataKanan(Rp(BH_TotalHutang), lebarNilai), _cfg.EscUkuranIsi)
        esc.CetakBaris("".PadRight(posLabel) & RataKanan("Dibayar      :", lebarLbl) & RataKanan(Rp(BH_NominalBayar), lebarNilai), _cfg.EscUkuranIsi)
        esc.CetakBaris("".PadRight(posLabel) & New String("="c, lebarKanan), _cfg.EscUkuranIsi)
        esc.CetakBaris("".PadRight(posLabel) & RataKanan("Sisa Hutang  :", lebarLbl) & RataKanan(Rp(BH_SisaHutang), lebarNilai), _cfg.EscUkuranIsi)
        esc.CetakGaris()
...
#Region "Cetak Dot Matrix"
    ...
    Private Function BangunEscP() As Byte()
        ...
        baris.Add(pI & garis)
        baris.Add(pI & KiriKanan("No Bayar", BH_NoBayar, n))
        baris.Add(pI & KiriKanan("Tanggal", BH_Tanggal.ToString("yyyy-MM-dd HH:mm:ss"), n))
        baris.Add(pI & KiriKanan("Kasir", BH_IdUser & " - " & BH_IdKomputer, n))
        baris.Add(pI & KiriKanan("Supplier", BH_NamaSupplier, n))
        baris.Add(pI & garis)
        baris.Add(pK & "BUKTI BAYAR HUTANG".PadLeft((nK + "BUKTI BAYAR HUTANG".Length) \ 2))
        baris.Add(pI & garis)
        ...
        baris.Add(pI & RataKiri("No Faktur Beli", lebarFaktur) & RataKanan("Dibayar", lebarBayar) & RataKanan("Sisa", lebarSisa))
        baris.Add(pI & garis)
        For Each item As ItemBayarHutang In BH_DaftarDetail
            Dim faktur As String = If(item.IdBeli.Length > lebarFaktur, item.IdBeli.Substring(0, lebarFaktur), item.IdBeli)
            baris.Add(pI & RataKiri(faktur, lebarFaktur) & RataKanan(BHRp(item.Pembayaran), lebarBayar) & RataKanan(BHRp(item.Sisa), lebarSisa))
        Next
        baris.Add(pI & garis)
        baris.Add(pI & KiriKanan("Total Hutang :", BHRp(BH_TotalHutang), n))
        baris.Add(pI & KiriKanan("Dibayar      :", BHRp(BH_NominalBayar), n))
        baris.Add(pI & KiriKanan("Sisa Hutang  :", BHRp(BH_SisaHutang), n))
        baris.Add(pI & garis)
