...
Public Class GdiCetakBayarPiutang
    ...
    Public Sub New()
        _cfg = New KonfigurasiThermal("BayarPiutang")
    End Sub
    ...
    Public Sub Cetak()
        ...
        Next
        If _cfg.KodeLaciKasir <> "(Tidak Ada)" Then BukaLaciKasir("BayarPiutang")
    End Sub
    ...
    Private Sub HitungPanjang()
        _panjangKertas = BP_DaftarDetail.Count * 25 + 350
    End Sub
    ...
    Private Sub CetakHalaman(e As PrintPageEventArgs)
        ...
        g.DrawString("No Bayar", FKet, Brushes.Black, BK, y) : g.DrawString(": " & BP_NoBayar, FKet, Brushes.Black, pv, y) : y += 10 + Jarak
        g.DrawString("Tanggal", FKet, Brushes.Black, BK, y) : g.DrawString(": " & BP_Tanggal.ToString("yyyy-MM-dd HH:mm:ss"), FKet, Brushes.Black, pv, y) : y += 10 + Jarak
        g.DrawString("Kasir", FKet, Brushes.Black, BK, y) : g.DrawString(": " & BP_IdUser & " - " & BP_IdKomputer, FKet, Brushes.Black, pv, y) : y += 10 + Jarak
        g.DrawString("Pelanggan", FKet, Brushes.Black, BK, y) : g.DrawString(": " & BP_NamaPelanggan, FKet, Brushes.Black, pv, y) : y += 14 + Jarak
        g.DrawString(Garis, FGaris, Brushes.Black, BK, y) : y += 10 + Jarak
        g.DrawString("BUKTI BAYAR PIUTANG", New Font(_cfg.FontKeterangan, _cfg.UkuranKeterangan + 2), Brushes.Black, Tengah, y, fmtTengah) : y += 14 + Jarak
        g.DrawString(Garis, FGaris, Brushes.Black, BK, y) : y += 5 + Jarak

        ' Kolom item
        Dim m3 As Integer = BK + CInt(LebarPx * 0.55)
        Dim m5 As Integer = BK + CInt(LebarPx * 0.95)
        g.DrawString("No Faktur Jual", FIsi, Brushes.Black, BK, y)
        g.DrawString("Dibayar", FIsi, Brushes.Black, m3, y, fmtKanan)
        g.DrawString("Sisa", FIsi, Brushes.Black, m5, y, fmtKanan) : y += 10 + Jarak
        g.DrawString(Garis, FGaris, Brushes.Black, BK, y)

        For Each item As ItemBayarPiutang In BP_DaftarDetail
            y += 10 + Jarak
            g.DrawString(item.IdJual, FIsi, Brushes.Black, BK, y)
            g.DrawString(Rp(item.Pembayaran), FIsi, Brushes.Black, m3, y, fmtKanan)
            g.DrawString(Rp(item.Sisa), FIsi, Brushes.Black, m5, y, fmtKanan)
            y += 10 + Jarak
            g.DrawString("  Tgl: " & item.TanggalJual.ToString("dd-MM-yyyy") & "  Status: " & item.Status,
                         New Font(_cfg.FontIsi, Math.Max(6, _cfg.UkuranIsi - 1)), Brushes.Gray, BK, y)
        Next
        y += 10 + Jarak
        g.DrawString(Garis, FGaris, Brushes.Black, BK, y) : y += 5 + Jarak

        ' Total
        g.DrawString("Total Piutang:", FIsi, Brushes.Black, m3, y, fmtKanan)
        g.DrawString(Rp(BP_TotalPiutang), FIsi, Brushes.Black, m5, y, fmtKanan) : y += 10 + Jarak
        g.DrawString("Dibayar      :", FIsi, Brushes.Black, m3, y, fmtKanan)
        g.DrawString(Rp(BP_NominalBayar), FIsi, Brushes.Black, m5, y, fmtKanan) : y += 10 + Jarak
        g.DrawString(BuatGarisGanda(HitungLebarGaris(_cfg.LebarKertas)), FGaris, Brushes.Black, m3, y) : y += 10 + Jarak
        g.DrawString("Sisa Piutang :", New Font(_cfg.FontIsi, _cfg.UkuranIsi, FontStyle.Bold), Brushes.Black, m3, y, fmtKanan)
        g.DrawString(Rp(BP_SisaPiutang), New Font(_cfg.FontIsi, _cfg.UkuranIsi, FontStyle.Bold), Brushes.Black, m5, y, fmtKanan) : y += 10 + Jarak
        g.DrawString(Garis, FGaris, Brushes.Black, BK, y) : y += 10 + Jarak
