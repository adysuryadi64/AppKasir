...
Module ModuleCetakBayarPiutangInkjet
    ...
    Private Sub Pd_PrintPage(sender As Object, e As PrintPageEventArgs)
        ...
        g.DrawString("NOTA PEMBAYARAN PIUTANG", New Font(_cfg.FontJudul, _cfg.UkuranJudul + 1, FontStyle.Bold), Brushes.Black, tengah, y, fmtTengah)
        ...
        g.DrawString("No. Bayar", fIsi, Brushes.Black, xKiri, y) : g.DrawString(": " & BP_NoBayar, fBold, Brushes.Black, xVal1, y)
        g.DrawString("Kasir", fIsi, Brushes.Black, xKanan2, y) : g.DrawString(": " & BP_IdUser, fIsi, Brushes.Black, xVal2, y) : y += lh
        g.DrawString("Tanggal", fIsi, Brushes.Black, xKiri, y) : g.DrawString(": " & BP_Tanggal.ToString("dd-MM-yyyy HH:mm:ss"), fIsi, Brushes.Black, xVal1, y) : y += lh
        ...
        g.DrawString("No Faktur Jual", fBold, Brushes.Black, xFaktur, y)
        g.DrawString("Tgl Jual", fBold, Brushes.Black, xTgl, y)
        g.DrawString("Total Piutang", fBold, Brushes.Black, xTotal, y, fmtKanan)
        ...
        For Each item As ItemBayarPiutang In BP_DaftarDetail
            ...
            g.DrawString(item.IdJual, fIsi, Brushes.Black, xFaktur, y)
            g.DrawString(item.TanggalJual.ToString("dd-MM-yyyy"), fIsi, Brushes.Black, xTgl, y)
            g.DrawString(item.Piutang.ToString("N0", cultureIndonesia), fIsi, Brushes.Black, xTotal, y, fmtKanan)
        ...
        Dim TulisLbl = Sub(lbl As String, fnt As Font, yy As Integer)
                       ...
                       End Sub
        TulisLbl("Total Piutang:", fIsi, y)
        g.DrawString(BP_TotalPiutang.ToString("N0", cultureIndonesia), fIsi, Brushes.Black, xSisa, y, fmtKanan) : y += lh
        TulisLbl("Dibayar      :", fIsi, y)
        g.DrawString(BP_NominalBayar.ToString("N0", cultureIndonesia), fIsi, Brushes.Black, xSisa, y, fmtKanan) : y += lh
        g.DrawLine(New Pen(Color.Black, 2), xLbl, y, xSisa, y) : y += 4
        TulisLbl("Sisa Piutang :", fBold, y)
        g.DrawString(BP_SisaPiutang.ToString("N0", cultureIndonesia), fBold, Brushes.Black, xSisa, y, fmtKanan) : y += lh + 8
        ...
        If _cfg.TampilTandaTangan Then
            ...
            g.DrawString("( " & BP_IdUser & " )", fKecil, Brushes.Black, xT2, y, fmtTengah) : y += lh + 8
        End If
